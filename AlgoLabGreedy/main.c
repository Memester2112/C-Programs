...
int optimal ( int n, int A[], int k )
{
    ...
    M = (int *)malloc((m+1) * sizeof(int));//Keeps tracks of the steps one needs to take to reach one
    for (i=0; i<=m; ++i)
    {
    ...
    {
        improved = 0;
        ++l;//counting number of iterations
        for (i=2; i<=m; ++i)
        {
            if (i % 2 == 0)     /* if i is even */
            {
                j = i / 2;   //option A:    /* one option is to divide i by 2 */
            ...
            {
                j = i + A[u]; //option B:   /* one option is to "reduce" i to j */
